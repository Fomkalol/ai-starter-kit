...
PAY=
# REVIEW_PAY=1 — включить раздел принудительно (SDK не из списка), REVIEW_PAY=0 — выключить.
if [ "${REVIEW_PAY:-}" = 1 ] || { [ "${REVIEW_PAY:-}" != 0 ] && git grep -q --untracked -I -i -E "$PAY_MARKERS" -- . ':(exclude)out' ':(exclude)*.lock' ':(exclude)*lock.json' ':(exclude)*.md' ':(exclude)*.css' ':(exclude)*review.sh' ':(exclude)*review-deep.sh' 2>/dev/null; }; then
  PAY=1
  PROMPT="$PROMPT
