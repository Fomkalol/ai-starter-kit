---
name: app-audit
description: "Глубокий аудит мобильного приложения по исходникам и видео: продукт, дизайн, надёжность. Используй для комплексного ревью приложения, а не отдельной правки экрана."
---
...
References загружай перед соответствующей частью, не все перед первым действием:

- `references/evidence-protocol.md` — перед сбором находок: типы доказательств, вопросы и ограничения выводов;
- `references/live-run-ios.md` — перед просмотром видео или запуском iOS: runtime evidence и simulator fallback;
- `references/directions.md` — перед профильными проходами и делегированием;
- `references/report-template.md` — перед синтезом: структура отчёта и финальная верификация.
...
Если repo slug неизвестен, не угадывать: возьми список репо из git-хостинга команды (API или `ls` по SSH), затем клонируй:

```bash
git clone <git-user>@<git-host>:<repo-path>/<slug>.git <scratchpad>/<app>
```

Slug может не совпадать с продуктом: Vinyl Appraiser лежал в `ios-vinyl-analysis`. Клон делать во временную папку вне `~/ai-workspace`.

### Вопросы

Задать максимум 5 вопросов, только если ответ меняет scope, severity или рекомендацию. Для каждого указать зависимый вывод.

- **Blocking:** невозможно определить приложение или нет ни одного пригодного источника для запрошенной проверки. Отсутствие только кода или только runtime-доказательств (видео либо безопасный живой прогон) не останавливает доступную часть; полный аудит без них не заявляй, покрытие опиши явно.
