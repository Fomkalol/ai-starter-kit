...
- Blocking: нельзя определить приложение или нет ни одного пригодного источника для запрошенной проверки. Если отсутствует только код либо runtime-доказательства (видео или безопасный живой прогон), продолжить доступную часть и явно отметить неполное покрытие.
